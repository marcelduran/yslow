/*global YUI*/
/*jslint browser: true, type: true, maxerr: 50, indent: 4 */
...
YUI({fetchCSS: false}).use('node', 'slider', function (Y) {
    ...
    if (url) {
        Y.one('#url').append(Y.Node.create('<a></a>')
            .setContent(url)
            .set('href', url)
        );
    }
    ...
    Y.Array.each(rules, function (rule) {
        ...
        val = Y.Node.create('<tr></tr>')
            ...
            .append('<td class="rule"><a href="/performance/rules.html#' +
                rule.link + '">' + rule.name + '</a></td>')
